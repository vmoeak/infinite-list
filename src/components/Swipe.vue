...
export default {
  ...
  mounted() {
    ...
    setTimeout(() => {
      //克隆dom
      this.starDom();
      this.dom.transform = `translate3d(${this.widthDom * -1}px, 0px, 0px)`;
      this.dom["-webkit-transform"] = `translate3d(${
        this.widthDom * -1
      }px, 0px, 0px)`;
      this.dom["-ms-transform"] = `translate3d(${
        this.widthDom * -1
      }px, 0px, 0px)`;
      if (this.autoPlay) {
        this.setTime();
  ...
  methods: {
    ...
    e() {
      if (this.slideing && this.t.s != -1) {
        ...
        x += this.t.m > 0 ? this.widthDom * 0.3 : this.widthDom * -0.3;
        this.index = Math.round(x / this.widthDom) * -1;
        this.wh("touch");
      }
    },
    setTransform(num) {
    ...
    wh(type) {
      ...
      this.setTransform(this.index * -1 * this.widthDom);
      this.t.m = 0;
      this.t.s = -1; //保证下次重新赋值
      if (this.autoPlay) {
        this.setTime();
    ...
    starDom() {
      ...
      if (this.slidesLength > 1) {
        ...
        this.widthDom = document.querySelector(
          "." + this.className
        ).offsetWidth;
